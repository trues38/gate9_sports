namespace :nba do
  ...
  task fetch_odds: :environment do
    ...
    # Fetch today + next 3 days for odds coverage
    (0..3).each do |day_offset|
      ...
      begin
        ...
        puts "Error fetching scoreboard for #{date_str}: #{e.message}"
        next
      end

      events = data["events"] || []
      puts "  #{date_str}: #{events.count} events"
      ...
      events.each do |event|
        ...
        home_abbr = abbr_map[home_comp.dig("team", "abbreviation")] || home_comp.dig("team", "abbreviation")
        away_abbr = abbr_map[away_comp.dig("team", "abbreviation")] || away_comp.dig("team", "abbreviation")

        # Find matching game — use UTC date range to avoid timezone mismatch
        event_time = Time.parse(event["date"]).utc
        day_start = event_time.beginning_of_day
        day_end = event_time.end_of_day

        game = Game.where(sport: sport)
                   .where(home_abbr: home_abbr, away_abbr: away_abbr)
                   .where(game_date: day_start..day_end)
                   .first
        ...
        if odds
          ...
          game.lines_captured_at = Time.current
          game.save!
          updated += 1
